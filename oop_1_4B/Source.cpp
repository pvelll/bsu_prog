...
void printList(Node* head, fstream& fout) {
    ...
    while (head != nullptr) {
        cout << "Name: " << head->student.name << ", Date of birth: " << head->student.dateOfBirth << ", Course: " << head->student.course << ", Marks: " << head->student.gpa << endl;
        fout.write(reinterpret_cast <char*>(&head->student.name ), sizeof(char)) ;
        fout.write(reinterpret_cast <char*>(&head->student.dateOfBirth ), sizeof(char));
        fout.write(reinterpret_cast <char*>(&head->student.course ), sizeof(int));
        fout.write(reinterpret_cast <char*>(&head->student.gpa), sizeof(float));
        head = head->next;
    }
}
